...
namespace JayI\Cortex\Models\Concerns;
 ...
 * are skipped.
 *
 * Entries a model declares on `$dispatchesEvents` itself win over the
 * derived ones.
 */
trait DispatchesModelEvents
{
    ...
    private static function deriveModelEvents(): array
    {
        ...
        foreach (self::$modelEventHooks as $hook) {
            $event = 'JayI\\Cortex\\Events\\Model\\'.$prefix.Str::ucfirst($hook).'Event';
